...
function renderReport(report){
  ...
  scoreNum.textContent = report.grade ? report.score + ' · ' + report.grade : report.score;
...
function renderResults(summary){
  ...
  if(post){
    ...
    const beforeLabel = beforeGrade ? `${before} · ${beforeGrade}` : `${before}`;
    const afterLabel = post.grade ? `${post.score} · ${post.grade}` : `${post.score}`;
    scoreHtml = `
